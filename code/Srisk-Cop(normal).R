...
for (i in 1:c){
  ...
  try(for (j in 3:(m-1)){
    ...
    for(t in (round(M*j)-50):round(M*j)){
      ...
      uri = rank(dcri)/(n+1)
      urm = rank(dcrm)/(n+1)
      try<-try(BiCopEst(uri,urm,family=1),silent=TRUE)
      if('try-error' %in% class(try)){delta[t] = 0 }
      else{Bi = BiCopEst(uri,urm,family=1)
      delta[t] = Bi[2]}
      abs[t] = qnorm(pt(ri[t]/sigmar[t],df = dfri))*qnorm(pt(rm[t]/sigmam[t],df = dfrm))
      if(t>(round(M*j)-41-1)){aveabs[t] = mean(abs[(t-9):t])}
    }
    delta = as.numeric(delta)
    deltatrans = log(delta[(round(M*j)-40):round(M*j)]+1)+log(-delta[(round(M*j)-40):round(M*j)]+1)
    deltalag = delta[(round(M*j)-41):(round(M*j)-1)]
    ave = aveabs[(round(M*j)-40):round(M*j)]
    reg = lm(deltatrans ~ 1+ave+deltalag)
    for(t in 1:h){
      abs[round(M*j)+t] = qnorm(pt(rt(1,df = dfri),df = dfri))*qnorm(pt(rt(1,df = dfrm),df = dfrm)) # uniform
      aveabs[round(M*j)+t] = mean(abs[(round(M*j)+t-9):(round(M*j)+t)])
    }
    for(t in (round(M*j)+1):(round(M*j)+h))
    {delta[t] = f(reg$coefficients[1]+reg$coefficients[2]*aveabs[t]+reg$coefficients[3]*delta[t-1])}
    simr = matrix(0, h, mc)
    simm = matrix(0, h, mc)
    ...
    for(x in 1:h){
      sim = BiCopSim(mc, family = 1, par = delta[round(M*j)+x])
      for(t in 1:mc){
...
write.csv(LRMES, file = "lrmes-cop(normal).csv")
write.csv(btest, file = "srisk-cop-btest(normal).csv")
write.csv(btestm, file = "srisk-cop-btestm(normal).csv")
write.csv(time, file = "srisk-cop-time(normal).csv")
write.csv(srisk, file = "srisk-cop(normal).csv")
